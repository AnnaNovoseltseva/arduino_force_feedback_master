...
ros::Publisher p("adc_xy", &adc_msg);
...
void loop() {
  unsigned int arr_x_force[10];
  unsigned int arr_y_force[10];
  unsigned int tot_x = 0;
  unsigned int tot_y = 0;
  for (int i=0; i<5; i++){
      ...
      unsigned int XForceData = readRegister(READ_CH1, 3);
      arr_x_force[i]= XForceData;
    
      //Read the ADC CH1 data
      unsigned int YForceData = readRegister(READ_CH0, 3);
      arr_y_force[i]= YForceData;
  ...
  for (int i=0; i<5; i++){
    tot_x += arr_x_force[i]/10;
    tot_y += arr_y_force[i]/10;
  }
  adc_msg.adc0 = tot_x*2;
  adc_msg.adc1 = tot_y*2;
  p.publish(&adc_msg);
  tot_x = 0;
  tot_y = 0;
  nh.spinOnce();
